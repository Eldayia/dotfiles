...
```batch
call modules\development.bat
```

**Télécharger gaming + multimédia :**
```batch
call modules\gaming.bat
call modules\multimedia.bat
```
...
1. Identifier le module approprié (ex: `modules\gaming.bat`)
...
1. Cloner le dépôt
2. Exécuter `downloadSoftware.bat`
3. Attendre la fin des téléchargements
4. Installer manuellement les logiciels requis
5. Installer les paquets téléchargés depuis `Downloads\`
...
call modules\gaming.bat
call modules\multimedia.bat
call modules\communication.bat
```
...
1. **Sauvegarde** : `downloadSoftware.bat.old` est conservé comme backup
...
5. **Ordre** : L'ordre d'exécution des modules peut être modifié dans `downloadSoftware.bat`
